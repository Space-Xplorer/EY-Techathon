...
export default function Pricing() {
  ...
  // ------------------------------------------------
  // Poll ONLY until pricing is ready
  // ------------------------------------------------
  useEffect(() => {
    if (!threadId) return;
    ...
    const poll = setInterval(async () => {
      try {
        ...
        // ✅ Stop polling ONLY when pricing is ready
        if (res.data?.pricing_detailed?.summary) {
          setIsLoading(false);
          clearInterval(poll);
        }
      } catch (err) {
        console.error("Pricing poll failed", err);
      }
    }, 2000);
  ...
  // Loading state
  // ------------------------------------------------
  if (isLoading || !state?.pricing_detailed?.summary) {
    return (
      <div className="min-h-screen gradient-bg flex items-center justify-center">
        <div className="card p-12 text-center">
          <Loader2 className="animate-spin text-cyan-400 mx-auto mb-4" size={32} />
          ...
          <p className="text-slate-400 mt-2">
            Analyzing specifications and commercial impact
          </p>
        </div>
      </div>
    );
  ...
  // Data
  // ------------------------------------------------
  const summary = state.pricing_detailed.summary;
  const commercialScore = state.commercial_score;
  const winProbability = state.winning_probability;
  ...
  const pricingItems = [
    { label: "Total Material", value: summary.total_material_cost_inr, icon: "📦" },
    { label: "Total Testing", value: summary.total_testing_cost_inr, icon: "🧪" },
    { label: "Subtotal", value: summary.subtotal_inr, icon: "📋", highlight: true },
    {
      label: "Contingency (10%)",
      value: summary.contingency_10pct_inr,
      icon: "⚠️",
      highlight: true,
    },
  ];
      ...
      <div className="max-w-4xl mx-auto px-6">
        ...
        <div className="mb-12">
          ...
          <p className="text-slate-400 text-lg">
            Cost breakdown and commercial assessment
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          {pricingItems.map((item, idx) => (
            <div
              ...
              className={`card p-6 flex items-center justify-between ${
                item.highlight
                  ? "bg-gradient-to-r from-slate-800 to-slate-900 border-cyan-500/30"
                  : ""
              }`}
            >
              <div className="flex items-center gap-4">
        ...
        <div className="card p-8 bg-gradient-to-r from-emerald-500/10 to-teal-500/10 border-emerald-500/30 mb-8">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-slate-400 mb-2">Grand Total</p>
              <p className="text-5xl font-bold text-emerald-400">
                ₹{formatCurrency(summary.grand_total_inr)}
              </p>
            </div>
            <DollarSign size={64} className="text-emerald-400/30" />
          </div>
        ...
        {(commercialScore || winProbability) && (
          <div className="card p-6 mb-8 bg-slate-900/80">
              ...
              {commercialScore != null && (
                <div className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                  ...
                  <span className="text-xl font-bold text-amber-400">
                    {Math.round(commercialScore * 100)}%
                  </span>
                </div>
              )}
            ...
            <p className="text-xs text-slate-500 mt-3">
              * Probability combines technical fit and commercial competitiveness
            </p>
          </div>
        )}
